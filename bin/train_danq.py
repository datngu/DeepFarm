...
parser = argparse.ArgumentParser(description = "Train DanQ...)", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
...
parser.add_argument("--out", default = "danq_model", help = "model output files - only best model saved")
...
## source model adapted from https://github.com/uci-cbcl/DanQ/blob/master/DanQ_train.py

def build_model(learning_rate):
    inp = Input(shape=(NUM_INPUT, 4))
    x = Conv1D(filters=320, kernel_size=26, activation="relu", padding="valid", name="conv1d_1")(inp)
    x = MaxPooling1D(pool_size=13, strides=13, name="maxpool_1")(x)
    x = Dropout(0.2, name="dropout_1")(x)
    x = Bidirectional(LSTM(320, return_sequences=True), name="bi_LSTM")(x)
