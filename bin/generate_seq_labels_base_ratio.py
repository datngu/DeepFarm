...
parser = argparse.ArgumentParser(description = "Generating seq lables (overlaped basepair counts as labels)", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
parser.add_argument("--input", nargs='+', help = "List of bed files after applying 'bedtools intersect'", required = True)
parser.add_argument("--out", default = "./label", help = "Output label matrix directory: each file 'chrid.txt.gz' contains lables for 1 chromosome, each column is a epigentic profile, lables are #base_OCR/#base_target_seq, each row is a sequence")
...
def create_lable_each_chrom(chr_ids, chr, out_fn = 'test.txt.gz'):
    ...
    for file, path in zip(files, paths):
        ids, ratios = process_file(path, chr)
        res.loc[ids, file] = ratios
        count += 1
