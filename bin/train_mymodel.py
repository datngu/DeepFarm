...
class ConvBlock(layers.Layer):
    def __init__(self, seq_len = 1000, filters = 512, kernel_size = 11, drop_rate = 0.2, out_embeding = 100):
        ...
        self.conv3 =  layers.Conv1D(filters = filters*2*2 , kernel_size = kernel_size, strides=1, padding='same', activation='relu', name = 'conv2')
        # con4 for reduce embeding size
        self.conv4 =  layers.Conv1D(filters = out_embeding , kernel_size = 1, strides=1, padding='same', activation='relu', name = 'conv3')
        
        self.max1 = layers.MaxPool1D(pool_size=2,strides=2, padding='valid', name = 'maxpool2')
        self.max2 = layers.MaxPool1D(pool_size=2,strides=2, padding='valid', name = 'maxpool1')
        self.max3 = layers.MaxPool1D(pool_size=2,strides=2, padding='valid', name = 'maxpool1')
...
def build_model(seq_len, n_track, filters = 320, num_heads = 11, ff_dim = 512, embeding = 64):
...
with strategy.scope():
    model = build_model(NUM_INPUT, NUM_OUTPUT, filters = 320, num_heads = 11, ff_dim = 512, embeding = 64)
